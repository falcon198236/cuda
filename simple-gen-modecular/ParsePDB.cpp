...
public:
	...
	VDWCovalent(int c, int v)
	{
		//data came from Rasmol in Rasmol units, this is the reason for the division by 250.0
		//described here http://www.umass.edu/microbio/rasmol/rasbonds.htm
		VDWRadius = (v / 250.0f);
		CovalentRadius = (c / 250.0f);
	}
};
...
// Function that initialized the VDWCovalent map
void ParsePDB::initVDWCovalentMap()
{
	static bool initialized = false;
	if (initialized == false)
	{
		//Obtained from Pymol using -> iterate (all), print(name + " %.2f" % vdw)
		VDW_List["O"] = 1.52f;
		VDW_List["N"] = 1.55f;//1.55
		VDW_List["C"] = 1.70f;
		VDW_List["P"] = 1.80f;
		VDW_List["Pa"] = 1.80f;
		VDW_List["Pb"] = 1.80f;
...
//	HETATM 2292 2H   HOH   550      23.244   1.464  47.034  1.00  0.00           H 
		...
		&& string[3]=='A' && string[4]=='T' && string[5]=='M') )
	{
		...
		char atomType[16] = { 0 };
		sscanf_s(proseq[numproseq].detailtype, "%s\n", atomType, sizeof(atomType));
		auto iter = VDW_List.find(atomType);
		if (iter != VDW_List.end())
		{
			proseq[numproseq].radius = iter->second;
		}else
			proseq[numproseq].radius = 1.7f;
...
bool ParsePDB::loadpdb(char *pdbfile, int atomnum)
{
	FILE	*filein;
	char	oneline[255];	
	bool modelend=false;
	limitchainnum = atomnum;
	...
	while(!feof(filein) && !modelend)
	{
		...
		if (limitchainnum > 0 && numproseq >= limitchainnum) break;
    }
	fclose(filein);
